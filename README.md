...
`start` 在当前进程完成 WireGuard 握手、TUN 模式的探测目标路由匹配后返回 `ready`，随后后台监护继续工作。临时网络故障会按退避持续重试，直到恢复或用户停止；本地资源清理失败、认证需要人工处理、不可恢复错误或进程崩溃恢复耗尽会保留失败状态。macOS 默认对需要处理的后台失败发送本机通知；可以用 `CORPLINK_NOTIFY=0 scripts/corplink-traffic.sh start` 关闭后台通知。
